	...
	\begin{enumerate}
		\item 
			\begin{itemize}
				...
				\item Denote such map by $\mathcal{F}$. We see for $A\in \mathrm{Mod}_{A},\ B\otimes_{A}M\in \mathrm{Mod}_{B}$,
					$$
					...
					.$$ 
			\end{itemize}
		\item From \emph{a} we know $B\otimes _{A}C$ is a $B$-module and $C$-module simultaneously, therefore we have
			$$
			b\otimes c=(bc)(1_{B}\otimes 1_{C})
			$$ 
			where we define the product of $b$ and  $c$ as a map $B\times C\longrightarrow B\times C$.\emph{(Roughly just regard it a Cartesian product.)}
			Now addtition and multiplication will be given by
			\begin{align*}
				b_1\otimes c_1+b_2\otimes c_2&=((b_1+b_2)(c_{1}+c_2))(1_B\otimes 1_{C})\\
							     &=(b_1+b_2)\otimes (c_1+c_2)
			.\end{align*}
			\begin{align*}
				b_1\otimes c_1\cdot b_2\otimes c_2&=((b_1b_2)(c_1c_2))(1_{B}\otimes 1_{C})\\
								  &=(b_1b_2)\otimes (c_1c_2)
			.\end{align*}
			Commutative, associative and distributive law can be checked easily. And the $0,\ 1$ of this ring will be
			$$
			0_{B}\otimes 0_{C},\ 1_{B}\otimes 1_{C}
			$$ 
			respectively.
	\end{enumerate}
\end{exercise}
